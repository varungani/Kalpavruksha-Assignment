...
void duplicate_player(Player *dest, Player src) {
    *dest = src;
}

void duplicate_batting_stats(TeamBattingStats *dest, TeamBattingStats src) {
    *dest = src;
}

void duplicate_metrics(PlayerMetrics *dest, PlayerMetrics src) {
    *dest = src;
}

int validate_id_bounds(int id) {
    if (id < 1 || id > 1500) return 1;
    return 0;
}

int find_player_index_by_id(Player roster[], int id, int count) {
...
void show_unused_ids() {
    ...
    for (int i = 1; i <= 1000; i++) {
        if (!id_tracker[i]) printf("%d ", i);
    }
}

int acquire_unique_id(Player roster[], int count) {
    int id, processed = 0;
    do {
        if (processed) {
...
int validate_name_syntax(char buffer[]) {
...
void input_player_name(char *buffer) {
    int processed = 0;
    do {
        if (processed) {
...
int verify_team_exists(char *query_team) {
...
void input_team_name(char *buffer) {
    int processed = 0;
    do {
        if (processed){
...
void select_role(char *buffer) {
    ...
    while (1) {
        ...
        scanf("%d", &option);
        switch (option) {
            case 1: strcpy(buffer, "Batsman"); return;
            case 2: strcpy(buffer, "Bowler"); return;
            case 3: strcpy(buffer, "All-rounder"); return;
...
void merge_id_subarrays(Player roster[], int l, int m, int r) {
    int n1 = m - l + 1;
    int n2 = r - m;
    Player left[n1];
    Player right[n2];
...
void execute_id_sort(Player roster[], int l, int r) {
    if (l < r) {
        int m = l + (r - l) / 2;
...
void merge_teams_alpha(Player roster[], int l, int m, int r) {
    int n1 = m - l + 1;
    int n2 = r - m;
    Player left[n1];
    Player right[n2];
...
void execute_team_sort(Player roster[], int l, int r) {
    if (l < r) {
        int m = l + (r - l) / 2;
...
void merge_stats_by_sr(TeamBattingStats stats[], int l, int m, int r) {
    int n1 = m - l + 1;
    int n2 = r - m;
...
void execute_sr_sort(TeamBattingStats stats[], int l, int r) {
    if (l < r) {
        int m = l + (r - l) / 2;
...
void merge_metrics_by_role(PlayerMetrics metrics[], int l, int m, int r) {
    int n1 = m - l + 1;
    int n2 = r - m;
...
void execute_metrics_sort(PlayerMetrics metrics[], int l, int r) {
    if (l < r) {
        int m = l + (r - l) / 2;
...
int find_team_start_index(Player roster[], int n, char team[]) {
...
int find_team_end_index(Player roster[], int n, char team[]) {
    int start = 0, end = n - 1, res = -1;
    while (start <= end) {
        int mid = start + (end - start) / 2;
        if (strcmp(roster[mid].team, team) == 0) {
...
void show_team_roster(Player roster[], int n) {
...
void analyze_team_batting_power(Player roster[], int n) {
...
int resolve_role_string(int opt, char buffer[]) {
    switch (opt) {
        case 1: strcpy(buffer, "Batsman"); break;
        case 2: strcpy(buffer, "Bowler"); break;
        case 3: strcpy(buffer, "All-rounder"); break;
        default: printf("Invalid choice"); return -1;
    }
    return 1;
}

void compute_performance_data(Player roster[], PlayerMetrics metrics[], int start, int end) {
...
int find_role_start_index(PlayerMetrics metrics[], int start, int end, int choice) {
    char query[50];
    if (resolve_role_string(choice, query) == -1) return -1;
    int res = -1;
    while (start <= end) {
        int mid = start + (end - start) / 2;
...
int find_role_end_index(PlayerMetrics metrics[], int start, int end, int choice) {
    char query[50];
...
void render_performance_table(PlayerMetrics metrics[], int start, int k, int choice) {
    char query[50];
    if (resolve_role_string(choice, query) == -1) return;
    printf("\nID\tName\tRole\tRuns\tAvg\tSR\tWkts\tER\tPerf.Index\n");
    printf("\n====================================================================================\n");
...
void show_top_performers(Player roster[], int n) {
    execute_team_sort(roster, 0, n - 1);
    char buffer[50];
    printf("\nEnter team Name: ");
    getchar();
...
void global_role_ranking(Player roster[], int n) {
    int choice;
    printf("\nEnter Role (1-Batsman, 2-Bowler, 3-All-rounder): ");
    scanf("%d", &choice);
...
int main() {
    ...
    while (1) {
        ...
        switch (option) {
            case 1: register_new_player(&roster, &current_size); break;
            case 2: show_team_roster(roster, current_size); break;
            case 3: analyze_team_batting_power(roster, current_size); break;
            case 4: show_top_performers(roster, current_size); break;
            case 5: global_role_ranking(roster, current_size); break;
            case 6: free(roster); exit(0);
        }
    }
    return 0;
}